...
**Findings:** This graph shows the linear relationship between juvenile hare weight and hind foot length, which shows a linear relationship that is not very strong. Simple linear regression showed that hind foot length does not do the best job at predicting snowshoe hare weight (p < 0.001, R^2 = `r round(hare_lm_glance$r.squared, 2)`) with an average slope of $\beta$ = `r round(hare_lm_tidy$estimate[2], 1)` g mm^-1^. This means that with one millimeter increase in hind foot length, we expect an average increase in hare weight of `r round(hare_lm_tidy$estimate[2], 1)` g. Hind foot length and hare weight are positively correlated, according to this linear regression (Pearson's *r* = `r round(hare_cor_tidy$estimate, 2)`, p < 0.001), but the relationship is not strong. In this case, the linear regression ignores if homoscedasticity is normal. The variance of standard error seems not homogeneous throughout the sample, especially for male snowshoe hares. I believe the data does not meet the conditions for a linear regression to accurately represent the relationship of hind foot length with hare weight.

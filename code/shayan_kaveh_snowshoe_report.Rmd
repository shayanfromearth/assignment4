---
title: "Snowshoe Hares Exploratory Report - Bonanza Creek Experimental Project"
author: "By: Shayan Kaveh"
date: "Nov. 10, 2021"
...
j_hares <- read_csv(here("data", "bonanza_hares.csv")) %>% 
...
##### Summary Statistics
<center>
**Table 1.** Juvenile Hare Trap Count Statistics
</center>
```{r, fig.align = 'center'}
...
#### Visualization of Juvenile Hare Weights
...
ggplot(data = gender_j_hares, 
       aes(x = sex, y = weight)) +
  geom_beeswarm(aes(color = sex)) + # have to attach library(ggbeeswarm) to be able to use beeswarm plots
...
#### Statistics of Juvenile Hare Weights by Sex & Site
...
**Findings:** This graph shows the linear relationship between juvenile hare weight and hind foot length, which shows a relatively linear relationship. Simple linear regression showed that hind foot length predicts snowshoe hare weight (p < 0.001, R^2 = `r round(hare_lm_glance$r.squared, 2)`) with an average slope of $\beta$ = `r round(hare_lm_tidy$estimate[2], 1)` g mm^-1^. This means that with one millimeter increase in hind foot length, we expect an average increase in hare weight of `r round(hare_lm_tidy$estimate[2], 1)` g. Hind foot length and hare weight are significantly positively correlated, according to this linear regression (Pearson's *r* = `r round(hare_cor_tidy$estimate, 2)`, p < 0.001). In this case, the linear regression ignores if homoscedasticity is normal. The variance of standard error seems not homogeneous throughout the sample, and I believe linear regression is not well met for a linear regression to represent the relationship of hind foot length with hare weight appropriately. 
...
- Juvenile hare hind foot length and weight have a relatively linear relationship, as defined by simple linear regression. The linear regressions shows suggests that we can expect on average that with every 1 mm increase in hind foot length that hare weight will increase by `r round(hare_lm_tidy$estimate[2], 1)` g mm^-1^.
